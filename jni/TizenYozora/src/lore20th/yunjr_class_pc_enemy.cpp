...
namespace
{
	void enemyAttackWithWeapon(PcEnemy* p_enemy)
	{
		...
		{
			yunjr::game::console::writeConsole(7, 2, p_enemy->getName(yunjr::PcEnemy::JOSA_SUB), " 빗맞추었다");
			return;
		}
		...
		if (p_player->isConscious())
		{
			...
			{
				yunjr::game::console::writeConsole(13, 4, p_enemy->getName(yunjr::PcEnemy::JOSA_SUB), " ", p_player->getName(yunjr::PcEnemy::JOSA_OBJ), " 공격했다");
				yunjr::game::console::writeConsole(7, 3, "그러나, ", p_player->getName(yunjr::PcEnemy::JOSA_SUB), " 적의 공격을 저지했다");
				return;
			}
		...
		{
			yunjr::game::console::writeConsole(13, 4, p_enemy->getName(yunjr::PcEnemy::JOSA_SUB), " ", p_player->getName(yunjr::PcEnemy::JOSA_OBJ), " 공격했다");
			yunjr::game::console::writeConsole(7, 3, "그러나, ", p_player->getName(yunjr::PcEnemy::JOSA_SUB), " 적의 공격을 방어했다");
			return;
		}
		...
		yunjr::game::console::writeConsole(13, 4, p_player->getName(yunjr::PcEnemy::JOSA_SUB), " ", p_enemy->getName(yunjr::PcEnemy::JOSA_NONE), "에게 공격 받았다");
		yunjr::game::console::writeConsole(5, 4, p_player->getName(yunjr::PcEnemy::JOSA_SUB), " @D", smutil::IntToStr(damage)(), "@@만큼의 피해를 입었다");
	}
		...
		case 1:
			{
				...
				{
					yunjr::game::console::writeConsole(7, 1, "독 공격은 실패했다");
					return;
				}

				if (smutil::random(20) < p_player->luck)
				{
					yunjr::game::console::writeConsole(7, 3, "그러나, ", p_player->getName(yunjr::PcEnemy::JOSA_SUB), " 독 공격을 피했다");
					return;
				}

				yunjr::game::console::writeConsole(4, 2, p_player->getName(yunjr::PcEnemy::JOSA_SUB), " 중독 되었다 !!");
		...
		case 2:
			{
				...
				{
					yunjr::game::console::writeConsole(7, 1, "치명적 공격은 실패했다");
					return;
				}

				if (smutil::random(20) < p_player->luck)
				{
					yunjr::game::console::writeConsole(7, 3, "그러나, ", p_player->getName(yunjr::PcEnemy::JOSA_SUB), " 치명적 공격을 피했다");
					return;
				}

				yunjr::game::console::writeConsole(4, 2, p_player->getName(yunjr::PcEnemy::JOSA_SUB), " 의식불명이 되었다 !!");
		...
		case 3:
			{
				...
				{
					yunjr::game::console::writeConsole(7, 1, "죽음의 공격은 실패했다");
					return;
				}

				if (smutil::random(20) < p_player->luck)
				{
					yunjr::game::console::writeConsole(7, 3, "그러나, ", p_player->getName(yunjr::PcEnemy::JOSA_SUB), " 죽음의 공격을 피했다");
					return;
				}

				yunjr::game::console::writeConsole(4, 2, p_player->getName(yunjr::PcEnemy::JOSA_SUB), " 죽었다 !!");
	...
	void enemyCastSpellWithSpecialAbility(PcEnemy* p_enemy)
	{
...

		{
			...
			if (p_enemy->special_cast_level > 1)
			{
				...
				if ((p_last_player->isValid()) && (num_not_dead < 7) && (smutil::random(5) == 0))
				{
					...
					*enemy[ix_enemy] << *p_last_player;
					...
					yunjr::game::console::writeConsole(13, 4, p_enemy->getName(yunjr::PcEnemy::JOSA_SUB2), " 독심술을 사용하여 ", enemy[ix_enemy]->getName(yunjr::PcEnemy::JOSA_OBJ), " 자기편으로 끌어들였다");
				}
			}
			...
			if (p_enemy->special_cast_level > 2)
			{
				...
				{
					yunjr::game::console::writeConsole(13, 4, p_enemy->getName(yunjr::PcEnemy::JOSA_SUB), " ", player[ix_player]->getName(yunjr::PcEnemy::JOSA_NONE), "에게 죽음의 공격을 시도했다");

					if (smutil::random(60) > p_enemy->agility)
						yunjr::game::console::writeConsole(7, 1, "죽음의 공격은 실패했다");
					else if (smutil::random(20) < player[ix_player]->luck)
						yunjr::game::console::writeConsole(7, 3, "그러나, ", player[ix_player]->getName(yunjr::PcEnemy::JOSA_SUB), "죽음의 공격을 피했다");
					else
					{
						yunjr::game::console::writeConsole(4, 2, player[ix_player]->getName(yunjr::PcEnemy::JOSA_SUB), " 죽었다!!");
